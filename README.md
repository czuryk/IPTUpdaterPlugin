# Magisk IPTables Updater Plugin
...
Use the adb shell command to navigate to the **/data/local/tmp/ipt_updater.log** file. This file contains information about the plugin status. Try updating the proxy and observe any changes. You can also run the command **iptables -L -t nat** to retrieve all entries and ensure that the iptables configuration is up to date.
